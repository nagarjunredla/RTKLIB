...
static void setU8(unsigned char *p, unsigned long  u) {memcpy(p,&u,8);}
...
extern int gen_ubx(const char *msg, unsigned char *buff)
{
    ...
    *q++=id[i];
    ...
    /* VALSET sanity check */
    if (isvalset) {
        if (narg == 6) narg = narg - 2;     /* Adjusting for key value addition */
        else return 0;
    }
    for (j=1;prm[i][j-1]||j<narg;j++) {
        switch (prm[i][j-1]) {
            ...
            case FU8 : setU8(q,j<narg?(unsigned long )atoi(args[j]):0); q+=8; break;
    ...
    if (isvalset) {
        ...
        if (strncmp(args[j+1],"CFG-",4)) return 0;

        for (k=0;*vcmd[k];k++) {
            if (!strcmp(args[j+1]+4,vcmd[k])) break;
        }

        if (!*vcmd[k]) return 0;

        setU4(q,(unsigned int  )vid[k]); q+=4;  /* Set key */

        /* Set value */
        switch (vprm[k]) {
            case FU1 : setU1(q,(unsigned char )atoi(args[j+2])); q+=1; break;
            case FU2 : setU2(q,(unsigned short)atoi(args[j+2])); q+=2; break;
            case FU4 : setU4(q,(unsigned int  )atoi(args[j+2])); q+=4; break;
            case FU8 : setU8(q,(unsigned long )atoi(args[j+2])); q+=8; break;
            case FI1 : setI1(q,(signed char   )atoi(args[j+2])); q+=1; break;
            case FI2 : setI2(q,(short         )atoi(args[j+2])); q+=2; break;
            case FI4 : setI4(q,(int           )atoi(args[j+2])); q+=4; break;
            case FR4 : setR4(q,(float         )atof(args[j+2])); q+=4; break;
            case FR8 : setR8(q,(double        )atof(args[j+2])); q+=8; break;
            case FS32: sprintf((char *)q,"%-32.32s",args[j+2]); q+=32; break;
            default  : setU1(q,(unsigned char )atoi(args[j+2])); q+=1; break;
        }
